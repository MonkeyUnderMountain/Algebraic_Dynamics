...
\subsection{Interpolation of analytic maps}
    ...
    \begin{theorem}[{ref.\cite[Theorem 1]{Poonen14interpolation}\ cf.\cite[Theorem 3.3]{BGT10DMLetale}}]\label{prop:Poonen14_p-adic_interpolation_of_iterates}
        ...
        Suppose that \(\Phi = (\Phi_1,\cdots,\Phi_d)\in \kk^\circ\{ \underline{T} \}^d\) satisfies \(r := r_p/\|\Phi - \id_E\| > 1\).
        Then there exists a function \(F \in \kk\{ \underline{T}, S/s \}^d\), \(1<s<r\), such that for each \(n \in \bbZ_{\geq 0}\) and each \(x \in E\),
        \[F(x, n) = \Phi^n(x).\]
    \end{theorem}
    \begin{proof}
        Consider the formal series
        \[ F(\underline{T}, S) := \sum_{n=0}^\infty \binom{S}{n} \Delta^n(\id_E)(\underline{T}) = \sum_{n=0}^\infty f_n. \]
        We only need to show that \(f_n \in \kk\{\underline{T}, S/s\}^d\) and \(\|f_n\| \to 0\) as \(n \to \infty\).
...
\subsection{Pick integral models}
    ...
    \begin{lemma}\label{lem:existence_of_good_model_for_smooth_things}
        Let \(f: X \ratmap X\) be a dominant rational self-map of a projective variety defined over a finitely generated field \(\kk\) over \(\bbQ\).
        
        Then there exists a subring \(R \subseteq \kk\) of finite type over \(\bbZ\), 
